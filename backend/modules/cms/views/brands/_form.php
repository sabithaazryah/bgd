...
<div class="brands-form form-inline">
    ...
    <div class="row">
        ...
        <div class='col-md-6 col-sm-6 col-xs-12 left_padd'>
            ...
            ])->hint('Dimensions : 150*150');
            ?>
            <?php
            if ($model->image != '' && $model->id != "") {
                $image = explode(',', $model->image);
                if (!empty($image)) {
                    echo '<img width="75" style="border: 2px solid #d2d2d2;margin-right:.5em;" src="' . Yii::$app->request->baseUrl . '/../images/brands/brand' . $model->id . '.' . $model->image . '?' . rand() . '" />';
                }
                ?>
                <br>
                <br>
